...
const CabecalhoEvento = ({EnviaModoEdicao, EnviaModoApagar, tituloEvento, dataEvento, horaInicio, horaFim, enderecoEvento, numeroEnderecoEvento, idEvento}: any) => {
  const dataFormatada = new Date(dataEvento).toISOString().slice(0, 10).split('-').reverse().join('/');
  const horaInicioFormatada = horaInicio.slice(0, 5);
  const horaFimFormatada = horaFim.slice(0, 5);
  const endereco = enderecoEvento || numeroEnderecoEvento ? `${enderecoEvento}, ${numeroEnderecoEvento}` : 'Endereço não definido';
        ...
        <div className="titulo-infos-eventos">
          <div className="titulo-informacoes">
            <div className="titulo-do-evento">
              <h1 className='cabecalho-evento__titulo' title={tituloEvento}>{tituloEvento}</h1>
            </div>
            <div className="informacoes-evento">
              ...
              <div className={`alinhamento-info-icone ${enderecoEvento || numeroEnderecoEvento ? '' : 'cabecalho-evento--sem-endereco'}`}>
                <svg xmlns="http://www.w3.org/2000/svg" width="21" height="21" viewBox="0 0 21 21" fill="none">
                  <path d="M10.4999 18.8996C10.4999 18.8996 17.0739 13.0561 17.0739 8.67352C17.0739 5.04285 14.1306 2.09961 10.4999 2.09961C6.86927 2.09961 3.92603 5.04285 3.92603 8.67352C3.92603 13.0561 10.4999 18.8996 10.4999 18.8996Z" stroke={`${enderecoEvento || numeroEnderecoEvento? '#55379D' : '#afafaf'}`} strokeWidth="2"/>
                  <path d="M12.6002 8.39974C12.6002 9.55954 11.66 10.4997 10.5002 10.4997C9.34041 10.4997 8.40021 9.55954 8.40021 8.39974C8.40021 7.23994 9.34041 6.29974 10.5002 6.29974C11.66 6.29974 12.6002 7.23994 12.6002 8.39974Z" stroke={`${enderecoEvento || numeroEnderecoEvento? '#55379D' : '#afafaf'}`} strokeWidth="2"/>
                </svg>
                {endereco}
              </div>
            </div>
